...
import { CustomKTIcon, CustomToast } from "../../../../../components";
...
import { UserQueryByIdInstance } from "@app/useCases/authentication/query/UserQueryById";
...
const SignInMethod: FC = () => {
  ...
  const { data: UserData, isLoading } = useQuery({
    queryKey: [QUERIES.User, currentUser?.user.id],
    queryFn: () =>
      currentUser &&
      UserQueryByIdInstance.getUserById(
    ...
    onSubmit: async (values) => {
      ...
      try {
        const isOldPasswordCorrect =
          await makeRemoteAuthentication().CheckOldPassword(
            authenticationURLEnum.CheckOldPassword +
        ...
        formData.append("id", String(isOldPasswordCorrect?.user?.id));
        formData.append("Password", values.newPassword);

        const updateResponse = await UserCommandInstance.UpdateUserPassword(
          authenticationURLEnum.UpdatePassword,
          formData
        );
        ...
        if (updateResponse) {
          ...
          formik2.setFieldError("currentPassword", "Failed to update password");
        }
      } catch (error) {
        formik2.setFieldError("currentPassword", error + "An error occurred");
        CustomToast(`Failed ${error} `, "error");
      } finally {
        setLoading2(false);
      }
    },
        ...
        <div className="card-body border-top p-9">
          <div className="d-flex flex-wrap align-items-center">
            ...
            >
              ...
              <div className="fw-bold text-gray-600">{UserData?.email}</div>
            </div>
