import { IUserQueryById, IUserData } from "@domain/entities";
...
export class UserQueryById implements IUserQueryById, IQuery {
  constructor(private readonly httpPostClient: IHttpClient) {}

  async getUserById(url: string, id: string): Promise<IUserData> {
    const newUrl = `${url}userId=${id}`;
...
export const UserQueryByIdInstance = new UserQueryById(makeAxiosHttpClient());
