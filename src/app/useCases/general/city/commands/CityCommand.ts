...
export class CityCommand implements ICityCommand, ICommand {
  constructor(private readonly httpPostClient: IHttpClient) { }

  async createCity(url: string, body: any): Promise<ICityBody[]> {
  ...
  async updateCity(url: string, body: any): Promise<ICityBody[]> {
  ...
  async deleteCity(url: string, id: number): Promise<ICityBody[]> {
  ...
  async multipleDeleteCity(url: string, ids: number[]): Promise<ICityBody> {
    const httpResponse = await this.httpPostClient.deleteRequest({
      url,
      body: ids,
    });
  ...
  async deleteImage(url: string, id: number): Promise<ICityBody> {
    const httpResponse = await this.httpPostClient.deleteRequest({
      url,
      body: {
        id,
...
export const CityCommandInstance = new CityCommand(makeAxiosHttpClient());
