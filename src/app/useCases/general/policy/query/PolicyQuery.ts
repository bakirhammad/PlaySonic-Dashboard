import { IPolicyBody, IPolicyData, IPolicyQuery } from "@domain/entities";
...
export class PolicyQuery implements IPolicyQuery, IQuery {
  constructor(private readonly httpPostClient: IHttpClient) {}

  async getPolicyList(url: string): Promise<IPolicyBody> {
  ...
  async getPolicyById(URL: string, id: number): Promise<IPolicyData> {
...
export const PolicyQueryInstance = new PolicyQuery(makeAxiosHttpClient());
