...
import { IPolicyCommand, IPolicyBody } from "@domain/entities";
...
export class PolicyCommand implements IPolicyCommand, ICommand {
  constructor(private readonly httpPostClient: IHttpClient) {}

  async createPolicy(url: string, body: any): Promise<IPolicyBody[]> {
  ...
  async updatePolicy(url: string, body: any): Promise<IPolicyBody[]> {
  ...
  async deletePolicy(url: string, id: number): Promise<IPolicyBody> {
...
export const PolicyCommandInstance = new PolicyCommand(makeAxiosHttpClient());
